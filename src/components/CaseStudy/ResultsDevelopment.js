  ...
  return (
    <section className="results-development">
      <div className="results-development__container">
        ...
        <div ref={sliderRef} className="results-development__slider">
          <div className={`results-development__slider-wrapper animate-on-scroll animate-on-scroll--scale-in ${sliderVisible ? 'is-visible' : ''}`}>
            <img
              src={data.images[currentImageIndex]}
              alt={`${data.title} - Slide ${currentImageIndex + 1}`}
              className="results-development__image"
            />
          </div>

          {data.images.length > 1 && (
            <div className={`results-development__controls animate-on-scroll animate-on-scroll--fade-in ${sliderVisible ? 'is-visible' : ''}`}>
              <button
        ...
        <div ref={contentRef} className="results-development__content">
          <span className={`results-development__label animate-on-scroll animate-on-scroll--fade-in ${contentVisible ? 'is-visible' : ''}`}>
            {data.label}
          </span>
          <h2 className={`results-development__title animate-on-scroll animate-on-scroll--fade-in-up ${contentVisible ? 'is-visible' : ''}`}>
            {data.title}
          </h2>

          {data.keyResults && (
            <div ref={resultsRef} className="results-development__results">
              <h3 className={`results-development__results-title animate-on-scroll animate-on-scroll--fade-in-up ${resultsVisible ? 'is-visible' : ''}`}>
                Key Results (so far)
              </h3>
              <ul className="results-development__results-list">
                {data.keyResults.map((result, index) => (
                  <li
                    key={index}
                    className={`results-development__result-item animate-on-scroll animate-on-scroll--fade-in-left animate-delay-${index + 1} ${resultsVisible ? 'is-visible' : ''}`}
                  >
          ...
          {data.ongoing && (
            <div className={`results-development__ongoing animate-on-scroll animate-on-scroll--fade-in-up ${resultsVisible ? 'is-visible' : ''}`}>
